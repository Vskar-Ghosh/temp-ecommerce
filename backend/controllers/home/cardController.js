...
class cardController {
  add_to_card = async (req, res) => {
    ...
    try {
      ...
      if (product) {
        responseReturn(res, 404, { error: "Product already added" });
      } else {
  ...
  get_card_products = async (req, res) => {
    ...
    try {
      ...
      for (let i = 0; i < outOfStockProduct.length; i++) {
        card_products_count =
          card_products_count + outOfStockProduct[i].quantity;
      }
      ...
      for (let i = 0; i < stockProduct.length; i++) {
        ...
        card_products_count = card_products_count + quantity;
  ...
  quantity_inc = async (req, res) => {
    ...
    try {
      ...
      const producta = await cardModel.findById(cardId);
      responseReturn(res, 200, { message: "success" });
    } catch (error) {
      console.log(error.message);
    }
